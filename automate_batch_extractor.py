...
def process_folder_structure(root_path: str, mode: str = "extract", fallback_category: str = "other"):
    ...
    1. Sort subfolders (like 'JEE Main - 2010') by year.
    2. Process subfolders one by one.
    3. Inside each subfolder, process PDFs in batches of 5.
    4. Save to year-specific JSON files (e.g. 'jee_main_2010.json').
    """
    root = Path(root_path)
        ...
        try:
            status = {
                ...
                "folders": [
                    {
                        "name": f.name,
                        "status": "done" if any(k.startswith(f.name) for k in processed_h) else "pending"
    ...
    # Base slug from root folder (e.g. 'JEE Main' -> 'jee_main')
    base_slug = detect_category(root.name) or fallback_category
    ...
    for folder in subfolders:
        ...
        logger.info(f"📁 Processing Year Folder: {folder.name} -> Target: {target_filename}")
        
        while True:
            # 2. Get next batch of 5
            ...
            update_global_status(folder, None, newly_processed, total_questions, subfolders, processed_files)
            
            for pdf_path in batch:
                file_key = f"{pdf_path.name}_{pdf_path.stat().st_size}"
                logger.info(f"  📥 Extracting: {pdf_path.name}")
                update_global_status(folder, pdf_path, newly_processed, total_questions, subfolders, processed_files)
    ...
    update_global_status(None, "✅ All tasks completed!", newly_processed, total_questions, subfolders, processed_files)
